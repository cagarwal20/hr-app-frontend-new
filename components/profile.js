...
export default function Profile({navigation,route}) {
  ...
  const getStaff = async (id) => {
    console.log(id)
    try{
      const response = await fetch(`${TRANSACTIONS_API_ROOT}get_profile/${id}`, {
        method: 'GET',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
  ...
  return (
      <SafeAreaView style={styles.container}>
          ...
          <Text  style={styles.profileTexttop}>First Name: {data.first_name}</Text>
          <Text  style={styles.profileText}>Last Name: {data.last_name}</Text>
          <Text  style={styles.profileText}>Amount owed/owned: {data.money}</Text>
          <Text  style={styles.profileText}>In Time: {data.in_time}</Text>
          <Text  style={styles.profileText}>Out time: {data.out_time}</Text>
          <Text  style={styles.profileText}>Status: {data.status}</Text>
          </View>
          <Image source={{
            uri: 'https://reactnative.dev/docs/assets/p_cat2.png',
          }}
          style={{width: 200, height: 200 ,  marginLeft:110 , marginTop:75}}/>
      </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  profileText:{
    textAlign:'center',color:'white' , fontSize:25,paddingTop:15,
    color:'orange'
  },
  profileTexttop:{
    textAlign:'center',color:'white' , fontSize:25,paddingTop:55,
    color:'orange'
  },
});
