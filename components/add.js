...
export default function Add({navigation}) {
  ...
  const addStaff = async(fname,lname) =>{
    try{
      const response = await fetch(`${TRANSACTIONS_API_ROOT}create_staff/`, {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
  ...
  return (
      <SafeAreaView style={styles.container}>
        <Text style={{color:'white' ,flex:0.15 , textAlign:'center' , paddingTop:'15%' , fontSize:35}}>Add Staff</Text>
          {/* <Text > Demo Form </Text>
      <View>
        ...
        <TextInput style={styles.userpass} placeholder='First Name'  onChangeText={text => setfname(text)}
/>
        <TextInput style={styles.userpass} placeholder='Last Name' onChangeText={text=>setlname(text)}/>
        <TouchableOpacity onPress={()=>addStaff(fname,lname)}><Text style={{color:'blue' , textAlign:'center' , paddingTop:'10%' , fontSize:35}}>Submit</Text></TouchableOpacity>
